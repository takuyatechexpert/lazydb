...
| `v` / `V` | Visual / VisualLine モードへ |
| `w` / `b` / `e` | 単語移動 |
| `^` / `gg` / `G` | 行頭非空白 / バッファ先頭 / バッファ末尾 |
| `x` / `dd` / `D` | カーソル下/行/行末まで削除（ヤンク付き） |
| `dw` / `diw` | 単語末まで / 単語まるごと削除 |
| `cc` / `C` / `cw` / `ciw` | 行/行末まで/単語末まで/単語まるごと削除 → Insert |
| `s` / `S` | 1文字削除 / 行クリア → Insert |
| `r{ch}` | カーソル下を1文字置換（モード維持） |
| `yy` / `Y` / `yw` / `yiw` | ヤンク（クリップボードへもコピー） |
| `p` / `P` | カーソル後 / 前にペースト |
| `J` / `~` | 行結合 / 大小反転 |
| `u` / `Ctrl+R` | undo / redo |
| `=` | クエリ全体をフォーマット |
| `/` → `Enter` / `Esc` | インクリメンタル検索（確定 / 取消） |
| `n` / `N` | 次 / 前のマッチへ |
| `Esc` | チョード待機をキャンセル |

### Editor Visual / VisualLine モード

| キー | 動作 |
|------|------|
| `h` `j` `k` `l` `w` `b` `e` `^` `$` `0` `gg` `G` 等 | 選択範囲を拡縮 |
| `o` | anchor とカーソルを入れ替え |
| `y` | 選択範囲をコピー（クリップボード両対応） → Normal |
| `d` / `x` | 選択範囲を削除 → Normal |
| `c` | 選択範囲を削除 → Insert |
| `~` | 選択範囲を大小反転 |
| `=` | バッファ全体をフォーマット |
| `v` / `V` | モード切替（Visual ↔ VisualLine） / 同モードで押すと Normal へ |
| `Esc` | Normal モードへ |
